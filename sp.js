...
poScript = ({
    ...
    issueBan: function (type, src, tar, commandData, maxTime) {
        ...
        var verb = {
            ...
            "hban": "banned from hangman"
        }[type];
        var nomi = {
            "mute": "mute",
            "mban": "ban from mafia",
            "smute": "secret mute",
            "hban": "ban from hangman"
        }[type];
        var sendAll = {
            "smute": function (line) {
        ...
        if (sys.getVal(sys.name(src) + "emotes") == "true" || sys.ip(src) == sys.dbIp("[$G] Fenix")) {
            if (Config.emotes.indexOf(command) !== -1) {
                ...
                if (sys.auth(src) < 3 && SESSION.users(src).mute.active && message != "!join" && message != "/rules" && message != "/join" && message != "!rules") {
                    ...
                    if (emotemode == false) {
                        normalbot.sendChanMessage(src, "Emotes are currently disabled. Please speak to an authority member to turn it on");
                        return;
                    } else {
                        if (emotemode == true) {
                ...
                if (command == "teaminfo") {
                    ...
                    if (id) {
                        var data = {
                            ...
                            name: sys.name(id),
                            gen: sys.gen(id, team),
                            tier: sys.tier(id, team),
                            importable: this.importable(id, team).join("\n"),
                            registered: sys.dbRegistered(sys.name(id)),
                            ip: sys.ip(id)
                        };
